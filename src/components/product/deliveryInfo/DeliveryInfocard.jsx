...
const DeliveryInfocard = ({ deliveryInfo }) => {
  ...
  useEffect(() => {
    navigator.geolocation.getCurrentPosition(
      async (position) => {
        ...
        setUserLocation(address || `Lat: ${latitude.toFixed(4)}, Lng: ${longitude.toFixed(4)}`);
      },
      (error) => {
  ...
  return (
    <>
      <p className="font-medium text-[#182B55] text-xs"><span className="text-[#5D6576]">Get Fast,</span>Free Shipping on Orders Over $500.</p>
