...
const StockBanner = ({
  ...
  return (
    <section className="relative h-[300px] overflow-hidden md:h-[400px] mt-10">
      {/* Background Image */}
      <img src={`${import.meta.env.VITE_SERVER_URL}/assets/${bannerData?.item.image.id}`} alt={altText} className="h-full w-full object-cover" />
      ...
      <div className="absolute inset-0 flex flex-col items-start justify-center mx-auto px-8 md:px-5 max-w-7xl ">
        <div className="relative space-y-4 text-left">
          <p className="text-[#3F66BC] text-lg font-semibold md:text-xl">{bannerData?.item.title}</p>
          <h1 className="text-[#182B55] text-4xl font-bold md:text-5xl">{bannerData?.item.subtitle}</h1>
          <Link
