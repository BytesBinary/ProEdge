...
const SearchAndAuth = ({ products }) => {
  ...
  const getMatchedText = (result, searchTerm) => {
    switch (result.matchType) {
      case "variation":
        return highlightMatch(
          result.variationName,
  ...
  return (
    <div className="w-full lg:hidden relative" ref={searchRef}>
      <form className="w-full mb-4" onSubmit={handleSearchSubmit}>
        <div className="relative">
          <input
            type="search"
            placeholder="Search products..."
        ...
        <div className="absolute top-full left-0 right-0 mt-1 bg-white rounded-lg shadow-lg z-50 max-h-80 overflow-y-auto">
          {searchResults.slice(0, 5).map((result, index) => (
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                <p className="text-sm font-medium text-gray-900 truncate">
                  {getMatchedText(result, searchTerm)}
                </p>
                <p className="text-xs text-gray-500 truncate">
                  {result.categoryName}
                </p>
