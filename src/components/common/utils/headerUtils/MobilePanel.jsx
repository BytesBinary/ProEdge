        ...
        <div className="flex flex-1 overflow-hidden">
          ...
          >
            {categories.map((category, index) => (
              ...
              >
                <img
                  src={`${import.meta.env.VITE_SERVER_URL}/assets/${category.image.id}`}
                  alt=""
                  aria-hidden="true"
          ...
          <main className="w-3/4 overflow-y-auto p-4">
            {selectedCategory ? (
              <div className="space-y-6">
                ...
                {/* Subcategories */}
                <div className="space-y-6">
                  {selectedCategory.sub_category?.map((subcategory, subIndex) => (
                    <section key={subcategory.id} className="space-y-2">
                      <h4 className="text-sm font-semibold text-gray-800 px-2">
                        {subcategory.subcategory_name}
                      </h4>
                      <ul className="space-y-1">
                        {subcategory.child_category?.map((child, childIndex) => (
                          <li key={child.id}>
                            <Link
                            ...
                            >
                              <span className="truncate">{child.child_category_name}</span>
